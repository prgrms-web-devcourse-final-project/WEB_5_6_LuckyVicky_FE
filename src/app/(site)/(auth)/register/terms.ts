...
export const TERMS_CONTENT: Record<
  'age' | 'terms' | 'privacy-required' | 'privacy-optional',
  ModalContent | null
> = {
  age: null,
  terms: {
...
① “캐시”라 함은 “몰”에서 재화 등을 구매하기 위하여 회원이 충전하거나 “몰”이 부여하는 서비스 전용 선불수단을 말합니다.

② “보너스 캐시”라 함은 이벤트·프로모션·보상 등으로 무상 부여되는 캐시를 말하며 현금환급 대상이 아닙니다. MVP 기간 동안에는 제공되지 않습니다.

③ “현금성 캐시”라 함은 회원이 유상으로 충전한 캐시를 말합니다. 현금환급 기능은 제공하지 않습니다.

④ “작가지갑(정산캐시)”라 함은 판매대금 정산을 위해 작가회원에게 적립되는 지갑을 말합니다.

제11의3조(캐시의 충전·사용·차감 순서)

① 회원은 “몰”이 정한 방법(각종 페이 등)으로 캐시를 충전할 수 있습니다.

② 캐시 사용 시 차감 순서는 보너스 캐시 → 현금성 캐시 순으로 합니다(별도 고지 시 달리 정할 수 있음).

③ 캐시는 “몰” 내 재화·용역 결제에 한하여 사용할 수 있으며, 타인에게 양도·대여·담보 제공할 수 없습니다.

제11의4조(캐시의 환급·소멸)

① 회원은 캐시를 현금으로 환급받을 수 없습니다.

② 회원이 계정을 탈퇴하는 경우 보유 캐시는 전액 소멸합니다. (소멸 예정 금액은 탈퇴 화면에 고지합니다.)

③ 캐시 충전 후 결제취소·환불이 발생하는 때에는 결제 취소 범위 내에서 캐시로 재적립하거나, 최초 결제수단으로의 환불 등 “몰”이 정한 기준에 따릅니다.

④ “몰”은 장기간 미이용 계정의 캐시 소멸 기준(예: 최종 사용일로부터 12개월)을 서비스 화면에 사전 고지할 수 있습니다. 보너스 캐시는 부여 시 고지된 유효기간 경과 시 자동 소멸됩니다.

제11의5조(오류 정정 및 제한)

① 시스템 오류, 결제사 취소·차지백, 부정 사용 등이 발생한 경우 “몰”은 해당 금액을 **정정(가감)**할 수 있으며, 필요한 경우 일시적으로 캐시 사용을 제한할 수 있습니다.

② 회원의 귀책 사유로 인한 부정 충전·부정 사용이 확인되면 “몰”은 캐시 회수, 거래 취소, 회원자격 제한 등 필요한 조치를 할 수 있습니다.

제11의6조(작가지갑(정산캐시))

① 작가회원의 판매대금은 정산 주기(예: 매월/매주) 및 검수·환불 보류 기간 경과 후 작가지갑에 적립됩니다.

② 작가회원은 본인 확인(KYC) 및 세무 정보 등록 후 “몰”이 정한 절차에 따라 현금 출금을 신청할 수 있습니다. 출금 수수료·최소/최대 한도는 화면에 고지합니다.

③ 정산 대상 금액은 판매금액에서 수수료, PG 수수료, 환불·차지백·분쟁 보류액 등을 차감한 금액으로 산정합니다. 보류액은 사유 해소 시점에 정산됩니다.

④ 허위 등록·저작권 침해 등으로 거래가 취소되는 경우, 이미 지급된 금액은 상계 또는 환수될 수 있습니다.

제11의7조(탈퇴 및 계정 정지 시 처리)

① 회원이 탈퇴를 신청하는 경우, 미해결 주문·분쟁·환불이 있는 때에는 해당 사유가 해소될 때까지 탈퇴가 보류될 수 있습니다.

② 탈퇴가 확정되면 제11의4조에 따라 캐시는 소멸되고, 개인정보는 법령 및 개인정보처리방침에 따라 파기 또는 분리보관합니다.

③ 작가회원의 경우, 미정산 금액은 본인 확인 완료 및 보류 사유 해소 후 출금 절차에 따릅니다.

제11의8조(약관 우선·변경 고지)

① 캐시·작가지갑 운영정책의 세부 사항(충전/사용/수수료/한도/유효기간 등)은 서비스 화면의 운영정책으로 정할 수 있으며, 본 약관과 상충하는 경우 본 약관을 우선합니다.

② “몰”은 캐시·작가지갑 정책을 변경하는 때에는 적용일자 및 주요 변경 내용을 7일(이용자에게 불리한 경우 30일) 이전에 공지합니다.
...
부 칙(시행일) 이 약관은 2025년 9월 20일부터 시행합니다.
`,
  },
  'privacy-required': {
    title: '개인정보 수집 및 이용 동의(필수)',
    body: '회사는 회원 서비스 제공을 위해 다음과 같은 개인정보를 수집하며, 목적 달성 후 지체 없이 파기합니다.\n\n1. 수집 항목: 이메일, 연락처\n2. 수집 목적: 서비스 이용에 따른 본인 확인, 고객 상담 및 민원 처리\n3. 보유 기간: 회원 탈퇴 후 30일 이내 또는 관련 법령에 따른 기간',
  },
  'privacy-optional': {
    title: '개인정보 수집 및 이용 동의(선택)',
    body: '선택 항목은 마케팅 정보 제공을 위해 활용되며, 동의하지 않으셔도 서비스 이용에는 제한이 없습니다.\n\n1. 수집 항목: 생년월일, 관심 카테고리\n2. 수집 목적: 맞춤형 콘텐츠 추천 및 광고 안내\n3. 보유 기간: 동의 철회 또는 회원 탈퇴 시까지',
  },
};
